...
class User(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(["group_id"], ["group.group_id"], name="fk_group_id"),
    )
...
def check_user(
    user_id: int, username: str, name: str, group_id: int, session_func: session.Session
) -> User:
...
def vote_user(
    ...
    vote_input = -1
    from_user = check_user(
        from_user_id, from_username, from_user_name, group_id, session_func
    )

    to_user = check_user(to_user_id, to_username, to_user_name, group_id, session_func)
